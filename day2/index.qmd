...
- コメントは**AIへの指示にも使える**（後で説明します）

## 日本語が表れない時の解決策

- もう一つのスクリプトを作成して、使う

![](../images/fix-ja-typing.png){height="350"}
...
- 今回は"Create a git repository"をクリック（gitについてまたあとで説明します）
...
- 今までは、スクリプトを書くのに相当な知識が必要だた
- でも、AIを使うと、すぐにコードでやりたいことを実行できるようになる

![](../images/copilot.png)

## Copilotの設定

- `Tools` ➡︎ `Global Options` ➡︎ `GitHub Copilot` をクリック
- `Enable GitHub Copilot`をクリックして、GitHubアカウントでログイン

![Copilotの設定](../images/copilot-menu.png)

## Copilotの使い方

- スクリプトにカーソルを置いて、**コメントで**やりたいことを書く
- 例えば、`# plot diamonds data`と書いてみましょう
- すると、Copilotがコードを提案してくれる
- そのコードを受け入れるには、<kbd class="kbc-button">Tab</kbd>を押す

## 今回のスクリプトの目的

- これから、`diamonds`データセットを使って、以下のような**グラフを作って**から、その**グラフを保存する**
...
- `diamonds.R`は`diamonds.png`というグラフを作った

. . .

- うっかりして`diamonds.png`を消してしまった！どうしよう！？
